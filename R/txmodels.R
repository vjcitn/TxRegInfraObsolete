...
txmodels = function(sym, gr,
 ...
 suggInst = function(pname) {
   if (!requireNamespace(pname)) stop(sprintf("please install package %s to use txmodels.", pname))
 }
 suggInst("biovizBase")
 suggInst("ensembldb")
 ...
 eg <- ensembldb::exonsBy(get(edb), by = "tx",
  filter = AnnotationFilter::AnnotationFilterList(AnnotationFilter::SeqNameFilter(c(seq)),
  AnnotationFilter::GeneIdFilter("ENS", "startsWith")),
 ...
 if (plot.it) Gviz::plotTracks(list(Gviz::GenomeAxisTrack(),grt), showId=TRUE)
 invisible(grt)
}
