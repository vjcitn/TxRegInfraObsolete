...
```{r lksb, cache=TRUE}
s1 = sbov(rme1, GRanges("chr17", IRanges(38e6,38.1e6)))
s1
dim(ca <- sparseAssay(s1, 3))  # compact gives stack overflow
head(ca[,c(7,18)])
```
...
car = strsplit(rownames(ca), ":|-")
an = as.numeric
gr = GRanges(ormm@chromosome, IRanges(an(sapply(car,"[", 2)), an(sapply(car,"[", 3))))
gr1 = gr
gr1$score = ca[,1]
gr2 = gr
gr2$score = ca[,2]
