...
```{r dogr}
library(graph)
s1$variant_id = as.character(s1$variant_id)
allg = unique(s1$symbol)
alls = unique(as.character(s1$variant_id))
g1 = new("graphNEL", edgemode="directed")
g1 = addNode(c(allg, alls), g1)
for (i in 1:length(s1)) {
  g1 = addEdge(s1[i]$variant_id, s1[i]$symbol, g1)
}
edgeDataDefaults(g1, "qvalue") = NA
edgeDataDefaults(g1, "slope") = NA
for (i in 1:length(s1)) {
  edgeData(g1, s1[i]$variant_id, s1[i]$symbol, "qvalue") = s1[i]$qvalue
  edgeData(g1, s1[i]$variant_id, s1[i]$symbol, "slope") = s1[i]$slope
  }
g1
```
